...
class Widget:
    ...
    def transmit(self, label, data):
        data_size = len(data)
        header = label + chr(data_size & 0xFF) + chr((data_size >> 8) & 0xFF)
        message = bytes(SOM + header + data + EOM)
        self.serialPort.write(message)
    
    def send_dmx(self, packet):
        msg = SC + b''.join(packet.data)
...
class Packet:
    def __init__(self, colors):
        ...
        for color in colors:
            r, g, b = color
            self.data.append(chr(int(r * 255)))
            self.data.append(chr(int(g * 255)))
            self.data.append(chr(int(b * 255)))
